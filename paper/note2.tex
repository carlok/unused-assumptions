...
\input{tables/note-macros}
\input{tables/density-macros}
...
\title{Assumptions that were never used}
...
A companion project measured the same question at a different strength, reading
only the \emph{root} of each proof term --- the lemma the proof ends on --- and
...
\section{The same rate in a library no human wrote}\label{sec:tauceti}

If the pattern were a habit of Mathlib's authors, a corpus written by a machine
should not share it. Tau Ceti \cite{tauceti} is such a corpus --- formal
mathematics directed by human-written roadmaps, implemented by AI contributors
and subject to adversarial review, incubated by the Lean FRO and the Mathlib
Initiative. Swept with the same hand-written table --- 42 of its 48 edges appear in that
...
A companion project counted, over the whole library at our revision, how many
declarations cite each survivor --- from proof terms, not from names. Of the
...
verdicts finds it --- only building the file does. It is the same lesson as the
descent floor: a stage that computes something must keep what proved it.

The \patchKept{} that survive are one-line changes against a named revision,
each with the file it belongs to, shipped in \texttt{patches/} with every
rejection and its first compiler error beside them. They are the smallest
actionable thing this work produces, and the only part of it a maintainer can
evaluate without reading the rest.
...
\paragraph{Nobody has said they want these.} None has been offered upstream. The
difference between a weakening that is true and one that is wanted is the whole
value of the result, and we have evidence for only the first. What we can offer
is \patchKept{} one-line patches that a maintainer can apply and check in a
minute each (Section~\ref{sec:patches}), and the evidence of
Section~\ref{sec:used} that these theorems are used rather than dead.
